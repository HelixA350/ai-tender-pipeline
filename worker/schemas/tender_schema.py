...
class ProcurementItemShort(BaseModel):
    """
    Позиция закупочной таблицы. Извлекай данные из любого формата документа —
    спецификации, ТЗ, таблицы Excel, текстового описания.
    Если значение не найдено — None.
    """

    no: int = Field(
        description=(
            "Порядковый номер позиции в списке закупки. "
            "Если в документе явно указан номер строки — используй его. "
            "Если нумерации нет — нумеруй сам, начиная с 1."
        )
    )

    request_number: Optional[str] = Field(
        description=(
            "Идентификатор запроса или заявки, к которой относится позиция. "
            "Буквенно-цифровой код, который объединяет группу позиций в один запрос — "
            "например, номер заявки на закупку или реестровый номер потребности. "
            "Если несколько позиций относятся к одному запросу — повторяй значение для каждой. "
            "Если не указан — None."
        ),
        default=None,
    )
    ...
    article: Optional[str] = Field(
        description=(
            "Партномер или артикул изделия, присвоенный производителем (OEM-код). "
            "Уникальный идентификатор конкретной детали в каталоге производителя. "
            "Копируй точно: сохраняй дефисы, пробелы, регистр. "
            "Не путать с моделью оборудования, в состав которого входит деталь. "
            "Если не указан — None."
        ),
        default=None,
    )
    ...
    name: str = Field(
        description=(
            "Полное наименование позиции — как оно указано в документе. "
            "Не сокращай, не перефразируй. "
            "Если технические характеристики являются частью наименования — включай их. "
            "Пример: 'Втулка подшипника GP MDP CS 50/60 SSIC'."
        )
    )

    qty: float = Field(
        description=(
            "Количество единиц данной позиции, которое требуется поставить. "
            "Числовое значение. Дробные значения сохраняй."
        )
    )
    ...
    unit: str = Field(
        description=(
            ...
            "Нормализуй к верхнему регистру без точек: 'шт.' → 'ШТ', 'м' → 'М', 'кг' → 'КГ'. "
            "Если единица измерения не указана явно, но из контекста очевидна — выведи её."
        )
    )

    brand: Optional[str] = Field(
        description=(
            "Торговая марка или бренд товара. "
            "Коммерческое название под которым продаётся изделие — "
            "может отличаться от названия завода-изготовителя. "
            "Если в документе указаны бренд и производитель через слэш — "
            "бери первый. Если не указан — None."
        ),
        default=None,
    )
    ...
    manufacturer: Optional[str] = Field(
        description=(
            "Завод или компания, которая физически производит изделие. "
            "Может совпадать с брендом или отличаться от него. "
            "Если в документе указаны через слэш (например, 'Magnochem / KSB') — "
            "бери второй как производителя. "
            "Если указан только один субъект без разделения — дублируй в оба поля. "
            "Если не указан — None."
        ),
        default=None,
    )
    ...
    equipment_model: Optional[str] = Field(
        description=(
            "Модель или тип оборудования, агрегата или установки, "
            "для которых предназначена данная позиция (запасная часть, расходник, компонент). "
            "Не путать с артикулом самой детали. "
            "Пример: 'MACD 080-050-2501 CC X2EIPR 160L2B', 'Caterpillar 3512'. "
            "Если не указана — None."
        ),
        default=None,
    )
    ...
    serial_number: Optional[str] = Field(
        description=(
            "Серийный номер конкретного экземпляра оборудования, "
            "для которого закупается позиция. "
            "Идентифицирует физический объект, а не модель. "
            "Копируй с префиксом, если он есть в документе. "
            "Если не указан — None."
        ),
        default=None,
    )

    drawing: Optional[str] = Field(
        description=(
            "Обозначение или номер чертежа, к которому относится позиция. "
            "Ссылка на конструкторскую документацию. "
            "Если не указан — None."
        ),
        default=None,
    )

    drawing_position: Optional[str] = Field(
        description=(
            "Номер позиции на чертеже или в сборочном составе. "
            "Указывает, где именно на схеме или в спецификации к чертежу "
            "находится данное изделие. "
            "Пример: 'поз.545.21'. Если не указана — None."
        ),
        default=None,
    )
    ...
    material: Optional[str] = Field(
        description=(
            ...
            "Актуально для деталей, где материал определяет применимость — "
            "уплотнения, втулки, трубы, фитинги. "
            "Пример: 'SSIC', 'нержавеющая сталь 316L', 'PTFE'. "
            "Если не указан — None."
        ),
        default=None,
    )
    ...
    comments: Optional[str] = Field(
        description=(
            "Любые дополнительные сведения по позиции, не вошедшие в другие поля: "
            "особые требования к поставке, уточнения по допустимым аналогам, "
            "примечания заказчика, ограничения. "
            "Копируй дословно или кратко резюмируй, если текст объёмный. "
            "Если нет — None."
        ),
        default=None,
    )
...
class TenderSchemaShort(BaseModel):
    """
    Короткая схема тендера для быстрого индексирования и отображения.
    Содержит семантическое резюме и таблицу закупки.
    """

    summary: "SemanticSummary"
    procurement_items: List[ProcurementItemShort] = Field(
        description=(
            "Полный список позиций закупки. "
            "Каждая отдельная позиция — отдельный объект в списке. "
            "Не пропускай позиции, не объединяй их. "
            "Сохраняй порядок как в документе."
        )
    )
...
class SemanticSummary(BaseModel):
    """
    Семантическая сводка тендера для векторного поиска похожих тендеров.
    Каждое поле — связное описание на русском языке, 1–3 предложения.
    Заполняй только на основе данных из документов; если данных нет — None.
    Не добавляй предположений и оценочных суждений.
    """

    customer: Optional[str] = Field(
        description=(
            "Краткое описание заказчика: отрасль, тип организации, регион. "
            "Пример: 'Нефтеперерабатывающий завод в Хабаровском крае (ПАО Роснефть).'"
        ),
        default=None,
    )
    ...
    procurement_method: Optional[str] = Field(
        description=(
            "Способ закупки и площадка. "
            "Пример: 'Запрос предложений на SAP SRM, переторг разрешён, лот неделим.'"
        ),
        default=None,
    )

    supply_scope: Optional[str] = Field(
        description=(
            "Суть поставки: что именно поставляется, сколько позиций, общий характер номенклатуры, "
            "ключевые производители и артикулы (если немного), единственный производитель или нет. "
            "Пример: 'Поставка 34 позиций запасных частей для дизельного двигателя Caterpillar 3512 "
            "(фильтры, прокладки, форсунки). Аналоги не допускаются. Все позиции от одного производителя.'"
        ),
        default=None,
    )

    service_scope: Optional[str] = Field(
        description=(
            "Описание работ (ШМР, ПНР, ТО, ремонт): что делается, на каком объекте/оборудовании, "
            "место выполнения, режим работы, минимальная численность, требования к допускам. "
            "Заполнять только если в тендере есть сервисная составляющая. "
            "Пример: 'Монтаж и пусконаладка насосного агрегата на объекте заказчика (г. Сургут), "
            "пн–пт 08:00–17:00, минимум 3 специалиста с допуском НАКС и группой ЭБ III.'"
        ),
        default=None,
    )

    engineering_scope: Optional[str] = Field(
        description=(
            "Описание инжиниринговой/конструкторской части: что нужно разработать или реинжинирить, "
            "стадии КД, требования к опытному образцу, передача прав. "
            "Заполнять только если есть реинжиниринг или разработка КД. "
            "Пример: 'Реинжиниринг гидравлического клапана с разработкой РКД в 2 экз. "
            "Опытный образец — 1 шт., испытательный стенд предоставляет заказчик. ИС передаётся заказчику.'"
        ),
        default=None,
    )
    ...
    delivery_terms: Optional[str] = Field(
        description=(
            "Условия поставки: Incoterms, место доставки, сроки поставки, "
            "допустимость досрочной поставки, удалённость / логистическая сложность. "
            "Пример: 'DDP склад заказчика (г. Нефтеюганск), срок поставки — 90 дней с даты заказа, "
            "досрочная поставка допускается.'"
        ),
        default=None,
    )

    financial_profile: Optional[str] = Field(
        description=(
            "Финансовые параметры: НМЦ (если есть), валюта, НДС, условия оплаты (тип, аванс %, срок). "
            "Не указывай ИНН, реквизиты. "
            "Пример: 'НМЦ 4,2 млн руб. с НДС 20%. Оплата по факту поставки в течение 30 дней с даты ТОРГ-12. Аванс не предусмотрен.'"
        ),
        default=None,
    )

    penalty_profile: Optional[str] = Field(
        description=(
            "Штрафные условия: процент пени за просрочку, база начисления, максимальный порог, "
            "условия расторжения. Описывай цифры точно как в документах. "
            "Пример: '0,1% от стоимости недопоставленного товара за каждый день просрочки, максимум 10%. "
            "Возможно одностороннее расторжение при просрочке свыше 30 дней.'"
        ),
        default=None,
    )
    ...
    product_requirements: Optional[str] = Field(
        description=(
            "Требования к продукту: состояние (новый/б.у.), гарантийный срок и точка отсчёта, "
            "допустимость аналогов и правила замены, требования импортозамещения, "
            "ограничения по стране происхождения. "
            "Пример: 'Только новое оборудование. Гарантия 24 месяца с даты ввода в эксплуатацию. "
            "Аналоги допускаются при подтверждении соответствия. Импортозамещение по ПП-719 не требуется.'"
        ),
        default=None,
    )

    participant_requirements: Optional[str] = Field(
        description=(
            "Требования к участнику: лицензии, СРО, опыт, ограничения по происхождению (МСП, только РФ и т.п.). "
            "Пример: 'Требуется членство в СРО (строительство), лицензия ФСБ на гостайну, "
            "опыт аналогичных поставок не менее 3 лет. Участие только для организаций РФ.'"
        ),
        default=None,
    )
    ...
    timeline_summary: Optional[str] = Field(
        description=(
            "Ключевые даты и сроки: срок подачи заявок, срок поставки/выполнения работ, "
            "общая продолжительность. Формат дат — человекочитаемый (например, '15 мая 2025'). "
            "Пример: 'Заявки принимаются до 15 мая 2025 (МСК). Срок поставки — 60 дней с даты договора.'"
        ),
        default=None,
    )

    complexity_flags: Optional[str] = Field(
        description=(
            "Перечень усложняющих факторов через точку с запятой — только те, что прямо следуют из документов. "
            "Возможные факторы: удалённый регион; жёсткая привязка к бренду; короткий срок подачи; "
            "требуются специальные допуски; сжатые сроки поставки; требования к опытному образцу; "
            "командировочные не компенсируются; нестандартная схема оплаты. "
            "Пример: 'Удалённый регион (Якутия); жёсткая привязка к бренду Siemens; "
            "срок подачи — 5 рабочих дней; требуется НАКС.'"
        ),
        default=None,
    )
    ...
    def to_text(self) -> str:
        """
        Склеивает все непустые поля в связный текст для векторного поиска.
        Порядок фиксирован; поля с None пропускаются.
        """
        sections = [
            ("Заказчик", self.customer),
            ("Способ закупки", self.procurement_method),
            ("Номенклатура", self.supply_scope),
            ("Сервисные работы", self.service_scope),
            ("Инжиниринг", self.engineering_scope),
            ("Условия поставки", self.delivery_terms),
            ("Финансы", self.financial_profile),
            ("Штрафы", self.penalty_profile),
            ("Требования к товару", self.product_requirements),
            ("Требования к участнику", self.participant_requirements),
            ("Сроки", self.timeline_summary),
            ("Особенности", self.complexity_flags),
        ]
